...
4. **Configure auth vars and D1** -- set `AUTH_PEPPER`, `ADMIN_HOST`, `PASSKEY_RP_ID`, and create/bind `AUTH_DB` in `wrangler.jsonc`.
5. **Bootstrap admin** -- open `/admin/login`, use first-time bootstrap to create the first super admin.

### Auth model

- End-user auth is first-party and tenant-scoped by host/domain.
- Login uses mailbox local part + fixed `@domain` suffix, with password and passkey support.
- Admin auth is a separate session realm under `/admin/login` with global mailbox visibility.
- MCP is restricted to admin session or `MCP_ADMIN_TOKEN`.
...
- **Auth:** First-party password + passkey sessions (user and admin realms)
...
- D1 database configured for the `AUTH_DB` binding

Domain users are isolated to their own mailbox/account in their resolved tenant domain. Admin users have explicit global visibility and control via the admin console.
